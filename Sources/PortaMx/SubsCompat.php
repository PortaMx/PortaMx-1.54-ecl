...
function pmx_eclnonemodal()
{
	...
	if(empty($_REQUEST['pmxcook']) && !WIRELESS && !empty($modSettings['pmx_eclmodal']) && !pmx_checkECL_Cookie())
	{
...
function pmx_checkads($state)
{
	...
	if(!empty($modSettings['pmx_eclmodal']) && !pmx_checkECL_Cookie() && $user_info['is_guest'] && !$user_info['possibly_robot'])
		return false;
	else
		return $state && true;
}
